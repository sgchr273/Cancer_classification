...
def save_accuracies(new_acc, filename, alg):
    ...
    try:
        ...
        acc_dict = {"entropy": [], "coreset": [], "fish":[],"bait":[], "rand" : [], "margin":[], "badge":[], "kmeans":[], "ADVdeepfool":[]}
    finally:
...
def save_imp_weights(new_imp_wt_idxs, filename,method):
    try:
        savefile = open("./Save/Imp_weights/imp_wts_idxs_"+ filename+'.p', "br")
        imp_wt_idxs = pickle.load(savefile)
        savefile.close()
    except:
        imp_wt_idxs =  {"standard": [], "dispersed": [], "relative":[]}
    finally:
        ...
        imp_wt_idxs[method].append(new_imp_wt_idxs)
