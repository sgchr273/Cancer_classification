...
class fisher_mask_sampling(Strategy):
    ...
    def calculate_fishmask(self, pct_top=0.02, method="standard"):
        ...
        if method == "standard":
            # print(sq_grads_expect.values.shape)
    ...
    def query(self, n):
        ...
        # print(sum(idxs_unlabeled), 'idxs_unlabeled')
        nClass = torch.max(self.Y).item() + 1
